...
class PeminjamController extends Controller
{
    ...
    public function alat(Request $request)
    {
        ...
        $alats = Alat::with([
                'category',
                'peminjaman' => function ($q2) {
                    $q2->latest('tanggal_kembali')->limit(1);
                }
            ])
    ...
    public function pinjamAlat(Request $request)
    {
        $request->validate([
            ...
            'kontak_peminjam' => 'required|max:15',
    ...
    public function material(Request $request)
    {
        ...
        $material = Material::with([
                'category',
                'pengambilan' => function ($q) {
                    $q->latest('tanggal_kembali')->limit(1);
                }
            ])    
            ->when($q, function ($query, $q) {
                $query->where(function ($sub) use ($q) {
    ...
    public function ambilMaterial(Request $request)
    {
        $request->validate([
            ...
            'kontak_pengambil' => 'required|string|max:20',
        ...
        foreach ($materials as $materialData) {
            ...
            PengambilanMaterial::create([
                ...
                'kontak_pengambil' => $request->kontak_pengambil,
                'tanggal_ambil' => now(),
    ...
    {
        $pengambilan = PengambilanMaterial::with('material')->findOrFail($id);

        if ($pengambilan->status !== 'diambil') {
            return back()->with('error', 'Material sudah diproses.');
        }

        $jumlahData = json_decode($request->input('jumlah_data', '{}'), true);
        $itemsData  = json_decode($request->input('items', '[]'), true);

        $jumlahKembali = $pengambilan->jumlah;
        ...
        if ($jumlahKembali < 0 || $jumlahKembali > $pengambilan->jumlah) {
            return back()->with('error', 'Jumlah pengembalian tidak valid.');
        }

        $habis = $pengambilan->jumlah - $jumlahKembali;
        ...
        return redirect()
            ->route('peminjam.pengembalian-material')
            ->with('success', "Dikembalikan: {$jumlahKembali} unit, Habis terpakai: {$habis} unit");
    }
    ...
    public function kembalikanMultipleMaterial(Request $request)
    {
        ...
        foreach ($items as $item) {
            $pengambilan = PengambilanMaterial::with('material')->findOrFail($item['id']);
            ...
            if ($jumlahKembali < 0 || $jumlahKembali > $pengambilan->jumlah) {
                return back()->with('error', "Jumlah {$pengambilan->material->name} tidak valid.");
            }
    ...
    public function riwayatAlat(Request $request)
    {
        $tanggal = $request->input('tanggal');
        $nama    = $request->input('nama');
        ...
        ->when($nama, function ($q) use ($nama) {
            $q->where('nama_peminjam', 'LIKE', "%{$nama}%");
        })
        ...
        return view('peminjam.riwayat-alat', compact('riwayatAlat', 'tanggal', 'nama'));
    }

    public function riwayatMaterial(Request $request)
    {
        $tanggal = $request->input('tanggal');
        $nama    = $request->input('nama');
        ...
        $riwayatMaterial = PengambilanMaterial::with('material')
            ->when($nama, function ($q) use ($nama) {
                $q->where('nama_pengambil', 'LIKE', "%{$nama}%");
            })
            ->when($tanggal, function ($q) use ($tanggal) {
                $q->whereDate('tanggal_ambil', $tanggal);
            })
            ->orderBy('tanggal_ambil', 'desc')
            ->paginate(10)
            ->withQueryString(); // biar pagination ga ilangin filter

        return view('peminjam.riwayat-material', compact(
            'riwayatMaterial',
            'tanggal',
            'nama'
        ));
