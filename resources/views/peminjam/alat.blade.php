            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Keterangan Pengembalian</th>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 align-top">
                                    <div class="text-sm text-gray-600 leading-relaxed max-w-xs">
                                        {{ optional($alat->peminjaman->first())->keterangan_kembali ?? '-' }}
                                    </div>
                                </td>
                        <td class="px-6 py-4 text-center">
                            @if($alat->available > 0)
...
<div id="modal-pinjam" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        ...
        <form id="form-pinjam" action="{{ route('peminjam.alat.pinjam') }}" method="POST">
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Kontak Peminjam</label>
                <input type="text" name="kontak_peminjam" id="kontak_peminjam" required 
                       class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                       placeholder="Misal: 6281234567890 (format: 62...)">
            </div>
