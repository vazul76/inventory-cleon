...
@if($nama)
    <!-- Riwayat Tools -->
    <div class="mb-8">
        <h3 class="text-2xl font-semibold mb-4"><i class="fas fa-tools mr-2"></i> Riwayat Peminjaman Alat</h3>
        
        @if($riwayatAlat->isEmpty())
            <div class="bg-gray-100 px-4 py-3 rounded">
                Tidak ada riwayat peminjaman Alat. 
            </div>
        @else
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tim Peminjam</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tool</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jumlah Pinjam</th>
        ...
        @else
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Tim Pengambil</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Perangkat</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jumlah Diambil</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jumlah Dikembalikan</th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr>
                            <td class="px-6 py-4">{{ $item->nama_pengambil }}</td>
                            ...
                            <td class="px-6 py-4">
                                @if($item->status === 'diambil')
                                    <span class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs">Diambil</span>
                                @elseif($item->status === 'dikembalikan')
                                    <span class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs">Dikembalikan</span>
                                @else
                                    <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">Dipakai</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
